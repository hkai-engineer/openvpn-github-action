...
function newClient() {
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		...
		case $PASS in
		1)
			EASYRSA_CRL_DAYS=3650 ./easyrsa --batch build-client-full "$CLIENT" nopass
			;;
		2)
			echo "⚠️ You will be asked for the client password below ⚠️"
			EASYRSA_CRL_DAYS=3650 ./easyrsa --batch build-client-full "$CLIENT"
			;;
		esac
...
function removeRevokedClient() {
	NUMBEROFCLIENTS=$(tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep -c "^R")
	if [[ $NUMBEROFCLIENTS == '0' ]]; then
		echo ""
		echo "You have no revoked clients!"
		exit 1
	fi

	echo ""
	echo "Select the revoked client you want to remove"
	tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep "^R" | cut -d '=' -f 2 | nl -s ') '
	until [[ $CLIENTNUMBER -ge 1 && $CLIENTNUMBER -le $NUMBEROFCLIENTS ]]; do
		if [[ $CLIENTNUMBER == '1' ]]; then
			read -rp "Select one client [1]: " CLIENTNUMBER
		else
	...
	CLIENT=$(tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep "^R" | cut -d '=' -f 2 | sed -n "$CLIENTNUMBER"p)
	cd /etc/openvpn/easy-rsa/pki/ || return
	sed -i -e "/^R.*=$CLIENT/d" index.txt
